...
export class QueueSettingsManager {
    ...
    async updateModes(modes: any) {
        if (typeof modes != "object" || !modes.length || modes.length < 1) return {
            error: true,
    ...
    async updateAdmins(admins: any, user:any) {
        if (!this.queue.isGroup) return {
            error: true,
            message: "This queue isn't a group!"
        };

        if (user._id != this.queue.owner) return {
            error: true,
            message: "This option is only avaliable for the group owner"
        }

        if (typeof admins != "object" || !admins.length || admins.length < 1) return {
            error: true,
        ...
        if (sanitizedAdmins.length < 1 || sanitizedAdmins.length > 50) return {
            error: true,
    ...
    async updateGenres(genres: any) {
        if (typeof genres != "object" || !genres.length || genres.length < 1) return {
            error: true,
    ...
    async updateTimeClose(timeclose: any) {
        if (typeof timeclose != "object") return {
            error: true,
            message: "Invalid form body!"
        };

        if (!timeclose.size || timeclose.size < 1 || timeclose.size > 30 || isNaN(Number(timeclose.size))) return {
            error: true,
            message: "Max schedule is 30 days!"
        }

        if (![true, false].includes(timeclose.enable)) return {
            error: true,
    ...
    async updateAutoClose(autoclose: any) {
        if (typeof autoclose != "object") return {
            error: true,
            message: "Invalid form body!"
        };

        if (!autoclose.size || autoclose.size < 1 || autoclose.size > 240 || isNaN(Number(autoclose.size))) return {
            error: true,
            message: "Max size is 240 requests!"
        }

        if (![true, false].includes(autoclose.enable)) return {
            error: true,
            message: "Invalid enabled status provided!"
        };
    ...
    async updateWebhook(webhook: any) {
        if (typeof webhook != "object" || webhook.url != "" && !webhook.url || !webhook.notify || !webhook.notify.length) return {
            error: true,
        ...
        if (webhook.notify.length < 1 || webhook.notify.length > validScopes.length) return {
            error: true,
        ...
        webhook.notify.forEach((scope) => {
            ...
            if (validScopes.includes(scope.trim().toLowerCase())) sanitizedScopes.push(scope.trim().toLowerCase())
        })

        if (sanitizedScopes.length < 1 || sanitizedScopes.length > validScopes.length) return {
            error: true,
            message: "You need to add one scope!"
        }
        ...
        if (webhook.url.trim().toLowerCase() != "") {
            try {
                const validHosts = ["canary.discord.com", "beta.discord.com", "discord.com"]
                const url = new URL(webhook.url);
    
                if (!validHosts.includes(url.hostname)) return {
                    error: true,
    ...
    async updateFilters(filters: any) {
        ...
        if (typeof filters.graveyard != "boolean" || typeof filters.wip != "boolean" || typeof filters.cross != "boolean")     return {
            error: true,
            message: "Invalid filter value provided! Filter values need to be a Boolean"
        };
